	...
	// executes code as normal if none of the error conditions are triggered
		...
		*numInts = fileSize / 4;
		...
		// Code continues if error message not triggered
			...
			// holder for read information from file
			int items;
			
			// for loop to pull information for array from file
			for (int i=0;i<*numInts;i++) {
				// reads items from the file one int at a time and assigns the value to items
				fread(&items, sizeof(int), 1, f);
				// assigns latest value to the current array index marked by i
				A[i] = items;
			}
			// closes file
			fclose(f);
